...
@app.get('/v2/translation/tokens', tags=['Translation'])
def tokenize(language, sentence_list, punctuations=None, stopwords=None):
    '''Tokenize any set of input sentences.
    Makes use of translation memory and stopwords for forming better phrase tokens'''
    return {}

@app.get('/v2/translation/token-translate', tags=['Translation'])
def token_translate(sentence_list, token_translations):
    '''Perform token replacement on provided sentences and 
    returns obtained drafts and draft_meta'''
    return []

@app.get('/v2/translation/auto-translate', tags=['Translation'])
def suggestion_translate(source_language, target_language, sentence_list,
    punctuations=None, stopwords=None):
    '''Attempts to tokenize sentences and prepare draft with autogenerated suggestions'''
    return []

@app.get('/v2/translation/suggestions', tags=['Translation'])
def get_suggestion(source_language, target_language, token, context, offset=None):
...
def add_alignments(source_language, target_language, alignments):
    '''Prepares training data with the alignments and update translation memory and suggestion models'''
    return
############## Autographa MT ##########################

@app.get('/v2/autographa/projects', tags=['Autographa'])
def get_projects(project_name=None, source_language=None, target_language=None, user_id=None ):
    '''Fetches the list of proejct and their details'''
    return []

@app.post('/v2/autographa/project', tags=['Autographa'])
def create_project(project_obj):
    '''Creates a new autographa MT project'''
    return {}

@app.put('/v2/autographa/project', tags=['Autographa'])
def add_books_to_project(project_obj):
    '''Adds more books to a autographa MT project's source'''
    return {}

@app.get('/v2/autographa/project/tokens', tags=['Autographa'])
def get_tokens(project_id, sentence_id_range=None, sentence_id_list=None):
    '''Tokenize the source texts'''
    return {}

@app.put('/v2/autographa/project/tokens', tags=['Autographa'])
def apply_token_translations(project_id, token_translations):
    '''Updates drafts using the provided token translations and returns updated verses'''
    return []

@app.get('/v2/autographa/project/draft', tags=['Autographa'])
def get_draft(project_id, sentence_id_list=None, sentence_id_range=None,
    auto_translate=False, format='plain-text'):
