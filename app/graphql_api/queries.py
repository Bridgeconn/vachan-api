...
class Query(graphene.ObjectType):
    ...
    suggest_translation = graphene.List(types.Sentence,
        ...
        sentence = graphene.List(types.SugggestTranslationInput))
    def resolve_suggest_translation(self, info, source_language, target_language, sentence):
        '''resolver'''
        db_ = info.context["request"].db_session
        sentence_list = sentence[0]["sentence_list"]
        if "stopwords" in sentence[0]:
            stopwords = sentence[0]["stopwords"]
        else:
            stopwords = None
        if "punctuations" in sentence[0]:
            punctuations = sentence[0]["punctuations"]
        else:
            punctuations = None
