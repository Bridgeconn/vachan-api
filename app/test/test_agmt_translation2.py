...
def test_draft_update_positive():
    ...
    resp = client.get(f"{UNIT_URL}/sentences?app_key={default_app_keys[TEST_APPS_LIST['AG']]}&project_id={project_id}",
    ...
    resp = client.put(f"/v2/autographa/project/draft?app_key={default_app_keys[TEST_APPS_LIST['AG']]}&project_id={project_id}",
        headers=headers_auth, json=put_data)
    sents = resp.json()
    assert len(sents) == 1
    ...
    resp = client.get(f"/v2/autographa/project/sentences?app_key={default_app_keys[TEST_APPS_LIST['AG']]}&"+\
...
def test_draft_update_negative():
    ...
    headers_auth['Authorization'] = "Bearer"+" "+initial_test_users['AgUser']['token']
    resp = client.put(f"/v2/autographa/project/draft?app_key={default_app_keys[TEST_APPS_LIST['AG']]}&project_id={project_id+1}",
        headers=headers_auth, json=put_data)
