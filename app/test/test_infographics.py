...
def test_post_incorrect_data():
    ...
    resource_name1 = resource_name.replace('infographic', 'info')
    data = []
...
def test_get_after_data_upload():
    ...
    # both title and book
    response = client.get(UNIT_URL+resource_name+"?book_code=gen&title=Noah's%20Ark",headers=headers_auth)
    assert response.status_code == 200
    assert len(response.json()) == 1
    ...
    response = client.get(UNIT_URL+resource_name+'?book_code=rev&title=vision',headers=headers_auth)
    assert_not_available_content(response)
...
def test_get_incorrect_data():
    ...
    resource_name = 'ur_TTT_1_infographic'
    response = client.get(UNIT_URL+resource_name+'?book_code=60',headers=headers_auth)
    assert_input_validation_error(response)

    response = client.get(UNIT_URL+resource_name+'?book_code=mark',headers=headers_auth)
    assert_input_validation_error(response)

    resp, resource_name = check_post([])
    assert resp.status_code == 201
    resource_name = resource_name.replace('infographic', 'graphics')
    response = client.get(UNIT_URL+resource_name, headers=headers_auth)
    assert response.status_code == 404
...
def test_put_incorrect_data():
    ...
    resource_name1 = resource_name.replace('infographic', 'graphics')
...
def test_delete_default():
    ...
    headers_auth = {"contentType": "application/json",#pylint: disable=redefined-outer-name
                "accept": "application/json"}
    headers_auth['Authorization'] = "Bearer"+" "+initial_test_users['VachanAdmin']['token']
    post_response = client.get(UNIT_URL+resource_name+"?book_code=gen&title=creation",\
        headers=headers_auth)
    assert post_response.status_code == 200
    assert len(post_response.json()) == 1
    for item in post_response.json():
        assert_positive_get(item)
    infographic_response = client.get(UNIT_URL+resource_name,headers=headers_auth)
    ...
    response = client.delete(UNIT_URL+resource_name + "?delete_id=" + str(infographic_id), headers=headers)
    assert response.status_code == 401
    assert response.json()['error'] == 'Authentication Error'
    ...
    for user in ['APIUser','AgAdmin','AgUser','VachanUser','BcsDev']:
        ...
        response = client.delete(UNIT_URL+resource_name + "?delete_id=" + str(infographic_id), headers=headers_au)
        assert response.status_code == 403
        assert response.json()['error'] == 'Permission Denied'
    ...
    response = client.delete(UNIT_URL+resource_name + "?delete_id=" + str(infographic_id), headers=headers_va)
    assert response.status_code == 200
    ...
    delete_response = client.get(UNIT_URL+resource_name+"?book_code=gen&title=creation",\
        headers=headers_auth)
...
def test_delete_default_superadmin():
    ...
    infographic_response = client.get(UNIT_URL+resource_name,headers=headers_sa)
    infographic_id = infographic_response.json()[0]['infographicId']


     #Delete infographic with Super Admin
    response = client.delete(UNIT_URL+resource_name + "?delete_id=" + str(infographic_id), headers=headers_sa)
    assert response.status_code == 200
    assert response.json()['message'] ==\
         f"Infographic id {infographic_id} deleted successfully"
    #Check infographic is deleted from table
...
def test_restore_default():
    ...
    restore_response =  client.get(UNIT_URL+resource_name+"?book_code=gen&title=creation",\
        headers=headers_auth)
