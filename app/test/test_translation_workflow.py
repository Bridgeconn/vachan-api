...
def test_end_to_end_translation():
    ...
    resp = client.post(BASE_URL+"mt/token-based/projects", headers=headers_auth, json=project_post_data)
    ...
    resp = client.put(BASE_URL+"mt/token-based/projects", headers=headers_auth, json=project_update_data)
    assert resp.json()['message'] == "Project updated successfully"

    # tokenize
    resp = client.get(BASE_URL+"mt/token-based/project/tokens?project_id="+str(project_id),headers=headers_auth)
    assert resp.status_code == 200

    # translate
    resp = client.put(BASE_URL+"mt/token-based/project/tokens?project_id="+str(project_id),
    ...
    NEW_USER_ID = initial_test_users['AgUser']['test_user_id']
    headers_auth['Authorization'] = "Bearer"+" "+initial_test_users['AgAdmin']['token']
    resp = client.post(BASE_URL+"mt/token-based/project/user?project_id="+str(project_id)+
    ...
    resp = client.put(BASE_URL+"mt/token-based/project/user", headers=headers_auth, json=user_data)
    # print(resp.json())
    assert resp.json()['message'] == "User updated in project successfully"

    resp = client.get(BASE_URL+"mt/token-based/projects?user_id="+str(NEW_USER_ID),headers=headers_auth)
    ...
    resp = client.put(BASE_URL+"mt/token-based/project/suggestions?project_id="+str(project_id)+
